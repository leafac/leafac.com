...
const murmurHash2 = murmurHash2Import
    .default;
...
if (process.env.TEST === "leafac--css") {
  ...
  `;
    ...
    assert.equal(pageLocalCSS(exampleCSS), "css--1ci2ui7");
    assert.equal(pageLocalCSS(css `
        font-family: "Public Sans";
      `), "css--l5tnu4");
    assert.equal(html `$${pageLocalCSS.toString()}`.trim(), 
    // prettier-ignore
    html `
      <style class="local-css">
...
export function cssClassName(css_) {
    const className = `css--${murmurHash2(css_)}`;
    if (!processedCSS.has(className))
        processedCSS.set(className, processCSS(css `
          ${`.${className}`.repeat(6)} {
            ${css_}
          }
        `));
...
export function css(inputParts, ...interpolations) {
    const outputParts = [];
    interpolations.push("");
    for (let index = 0; index < inputParts.length; index++) {
        ...
        if (Array.isArray(interpolation))
            outputParts.push(...interpolation);
        else
            outputParts.push(interpolation);
    }
...
if (process.env.PREPARE === "leafac--css")
    await fs.writeFile(url.fileURLToPath(new URL("./browser.css", import.meta.url)), processCSS(css `
      /* RESET */
