...
The part that reads `[· ... ● ... ○ ... ● ... · ...]` means “a sequence of zero or more spaces (`·`), followed by a sequence of zero or more pegs (`●`), followed by a sequence of zero or more holes (`○`), followed by a sequence of zero or more pegs (`●`), followed by a sequence of zero or more spaces (`·`).” This matches a single row of our initial board, regardless of whether it is the first row, which includes spaces but no holes, or the fourth row, which includes a hole but no spaces. The part that reads `([___] ...)` means “zero or more rows”.
