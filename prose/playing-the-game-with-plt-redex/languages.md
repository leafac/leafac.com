...
The term above does not represent a Peg Solitaire board: it is too small and the rows have different sizes. Yet, the `board` in the `peg-solitaire` language matches it. We could refine the language definition so that it matches *exactly* the terms that represent Peg Solitaire elements, but it would become more complicated and fail to communicate our intent to the readers. The named patterns we introduced in `peg-solitaire` will serve well for the definitions in the following sections, so this simpler language specification suffices. We will proceed assuming all boards are well-formed—if necessary, we could have defined a well-formedness check in the form of a [predicate relation](predicate-relations) that would narrow down even further the kinds of terms representing Peg Solitaire elements.
