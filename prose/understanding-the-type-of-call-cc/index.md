...
We can replace the `5` in this expression with an use of `call/cc` without changing its output:

<pre>
> (+ 1 <mark>(call/cc (λ (k) 5))</mark>)
6
</pre>
...
2. `call/cc` returns `5`, a number: <code>call/cc : ___ → <mark>Number</mark></code>.
