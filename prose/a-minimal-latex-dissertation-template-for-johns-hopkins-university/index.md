...
Do not fall into the trap of using LaTeX for everything in your document. It may be less productive and the results may not be the best. Instead of drawing illustrations and diagrams in TikZ, use [Inkscape](https://inkscape.org)—it can even export to LaTeX. Instead of plotting graphs with PGFPlots, use [macOS Numbers](https://www.apple.com/numbers/) or [Excel](https://products.office.com/en-us/excel). Pay attention to details including fonts and sizes so that the final document looks consistent. If you need mathematical formulas in these other programs, use [LaTeXiT](https://www.chachatelier.fr/latexit/).
...
Next, keep your TeX installation updated. Update in the middle of every year when there are new releases of the TeX distributions. You may even use the [TeX Live Utility](https://amaxwell.github.io/tlutility/) or the [TeX Live package manager](https://www.tug.org/texlive/tlmgr.html) to update CTAN packages every few months.
...
To learn LaTeX, read the parts of the [Wikibook](https://en.wikibooks.org/wiki/LaTeX) that may be relevant to you. Avoid other books that may be too deep to be useful to beginners, including the official LaTeX documentation, by Leslie Lamport; *The LaTeX Companion*; and *The TeXbook*, by Donald E. Knuth.
...
If you have similar needs, or if you just want to use a font that is not Computer Modern—the default LaTeX font that is already overused in academia—start by switching from the plain TeX engine to LuaTeX. In LaTeX Workshop (and other LaTeX editors), you can do this by including the following snippet as the first lines in your LaTeX source:<label class="margin-note"><input type="checkbox"><span markdown="1">When we select a different TeX engine we also have to be explicit about BibTeX as the bibliography manager.</span></label>
