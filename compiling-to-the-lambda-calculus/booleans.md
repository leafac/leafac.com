...
With the [architecture](architecture) in place, we are ready to start defining the `expand` clauses that will expand the forms from the [surface language](surface-language) into the [core language](core-language). We begin with one of the simplest kind of values: booleans. Our surface language includes boolean values (`#t` and `#f`), operators (`and`, `or`, `not`, and `xor`), and one conditional control-flow operator (`if`).
...
```racket
[`(if ,eᶜ ,eᵗ ,eᵉ) `(,eᶜ ,eᵗ ,eᵉ)] ;; INCORRECT
```
