...
◊publication/paper{
 ...
 ◊publication/paper/abstract{Demand-Driven Program Analysis (◊initialism{DDPA}) is a context-sensitive control-flow analysis for higher-order functional programming languages in the same space as k-◊initialism{CFA} and related analyses. ◊initialism{DDPA} is provably sound and polynomial, but that alone does not guarantee its applicability to real-world programs. This project studies ◊initialism{DDPA}’s practicality in two dimensions: expressiveness (the analysis closely approximates the run-time) and performance (the analysis is fast to compute). To address expressiveness, we extended the analysis to directly support recursion, eliminating the need for encodings. To address performance, we developed a higher-level programming language that compiles to ◊initialism{DDPA}’s core language, ported over benchmarks that exercise all its features, and measured the reference implementation’s performance, comparing to other state-of-the-art program analyses. ◊initialism{DDPA} is competitive and, in some cases, faster than the alternatives.}
}
...
◊publication/paper{
 ...
 ◊publication/paper/date{2016}
 ◊publication/paper/abstract{This artifact is a proof-of-concept implementation of ◊initialism{DDPA}, an on-demand program analysis for higher-order functional programs. The implementation, written in OCaml, includes a parser, evaluator, and ◊initialism{DDPA} analysis for the language defined in the companion paper (including the proper record semantics extension). The analysis may be performed using different levels of precision as specified by the user and is capable of rendering the control flow graphs and pushdown systems using the GraphViz language ◊initialism{DOT}. This artifact was used to verify the conclusions of the companion paper and produces visualizations matching those figures in the companion paper’s overview.}
}
...
◊publication/paper{
 ...
 ◊publication/paper/authors{Leandro Facchinetti, Zachary Palmer and Scott F. Smith}
 ◊publication/paper/venue{◊link["http://2016.ecoop.org/"]{30th European Conference on Object-Oriented Programming (◊initialism{ECOOP})}}
 ◊publication/paper/date{2016}
 ◊publication/paper/abstract{This artifact is a proof-of-concept implementation of ◊initialism{DDPA}, an on-demand program analysis for higher-order functional programs. The implementation, written in OCaml, includes a parser, evaluator, and ◊initialism{DDPA} analysis for the language defined in the companion paper (including the proper record semantics extension). The analysis may be performed using different levels of precision as specified by the user and is capable of rendering the control flow graphs and pushdown systems using the GraphViz language ◊initialism{DOT}. This artifact was used to verify the conclusions of the companion paper and produces visualizations matching those figures in the companion paper’s overview.}
}
...
◊publication/paper{
 ...
 ◊publication/paper/venue{◊link["http://tfp2016.org/"]{The 17th Symposium on Trends in Functional Programming (◊initialism{TFP} 2016)}}
 ◊publication/paper/date{2016}
...
◊publication/paper{
 ...
 ◊publication/paper/venue{Internal Workshop of ◊initialism{INCT-SEC}}
...
◊publication/paper{
 ...
 ◊publication/paper/authors{Leandro Facchinetti and Fernando Santos Osório}
 ◊publication/paper/venue{◊link["http://www.jointconference.fei.edu.br/wci/index.html"]{◊roman-number{III} Workshop on Computational Intelligence—◊initialism{WCI}. Joint Conference 2010—◊initialism{SBIA}—◊initialism{SBRN}—◊initialism{JRI}}}
...
◊publication/paper{
 ...
 ◊publication/paper/authors{Leandro Facchinetti and Fernando Santos Osório}
 ◊publication/paper/venue{◊foreign{◊roman-number{IV} Workshop de Iniciação Científica e Tecnológica de Computação (◊initialism{WICT})} (◊roman-number{IV} Scientific and Technologic Initiation Workshop)}
...
◊service/reviewer{
 ◊service/reviewer/title{◊link["http://csur.acm.org/"]{◊initialism{ACM} Computing Surveys (◊initialism{CSUR})}}
