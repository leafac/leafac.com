...
◊define-meta[date]{2017-08-04}

◊margin-note{◊no-indent[] I personally experienced all the issues listed here in a recent email migration.}

◊margin-note{This article assumes familiarity with the command-line.}

◊new-thought{Migration tools} in email clients do not work. Most applications, for example, ◊link["https://www.mozilla.org/en-US/thunderbird/"]{Thunderbird} and ◊link["https://support.apple.com/mail"]{Apple Mail}, come with assistants to import the email history from other applications. These features do not work: they lose email, concatenate all history in a single giant email, or create a new mailbox for each individual message, which break the organization system. This was supposed to be a solved problem, there are standard email exchange formats, for example, ◊code/inline{.eml} and ◊code/inline{.mbox} files. Unfortunately, every application has a slightly different understanding of those formats, so they do not solve the problem. It is a ◊emphasis{ridiculous} problem.

Many tools for converting between message formats exist in the Internet, but they are generally not well maintained or they are paid products. There is an alternative, simple solution, though: while email clients might not agree on the storage formats, they must all talk the same protocols to email servers! The source client can upload messages to the server, and the target client can download them. This method might not preserve tags and other client-specific advanced features. But it preserves the ◊emphasis{read} status, the folder structure, and other fundamental attributes of email archives. Moreover, this method is guaranteed to work regardless of the email clients involved.

In fact, people on the Internet have suggested this method before. And they recommend using the email provider the person already has, for example, ◊link["https://www.google.com/gmail/"]{Gmail} or ◊link["https://www.icloud.com"]{iCloud}. This is simple to setup, because it just reuses the configuration already in the email client. For this simplicity, it is most recommended approach to people not familiar with the command-line. But it has an important drawback: it involves uploading and re-downloading the whole message history! Depending on the size of the history, this can take days. It is a ◊emphasis{ridiculous} solution.

◊paragraph-separation[]

◊new-thought{In this article}, we introduce an alternative that brings together the two non-solutions discussed above. ◊emphasis{The ultimate solution to the ridiculous problem of email migration}. It is a compromise between running local conversion tools and using an email server to intermediate the migration: ◊emphasis{setup a local email server}.

◊full-width{◊svg{migration.svg}}

◊section['setup]{Setup}

◊new-thought{Backup the mailboxes}! A poorly setup migration might result in data loss!
...
Create a directory to hold the temporary Dovecot’s mailboxes:
...
◊new-thought{Configure Dovecot} with a ◊path{dovecot.conf} file, by  replacing the ◊code/inline{◊placeholder{placeholders}} on the following template:
...
◊margin-note{
  ...
  The ◊code/inline{-F} flag for Dovecot makes it run on the foreground. This makes it easy to see the logs as the server runs, and to kill it with ◊keyboard{Ctrl} + ◊keyboard{C}.
}
...
◊margin-note{Some email clients insist on having a server to send emails (◊acronym{SMTP}) during account creation. Let this part of the configuration fail or reuse the outgoing email configuration from other accounts.}
...
On the source email client, move emails from the local folders to the temporary server. Wait for completion and then, on the target email client, move emails from the temporary server to local folders. Wait for completion again. To guarantee that the process succeeded, close the email clients so that they commit pending transactions with the server and check the ◊path{◊placeholder{migration-directory}}, it should contain only empty directories and Dovecot’s administration files.

◊section['teardown]{Teardown}

◊new-thought{After the migration} is complete, remove the configuration connecting the email clients to the temporary email server. Then, stop the server by killing the process or running the following command on a separate terminal:

◊code/block{
$ sudo doveadm stop
}

Finally, remove the ◊path{◊placeholder{migration-directory}} and uninstall Dovecot:
...
Dovecot supports many kinds of services, for example, the ◊acronym{POP3} email server protocol and authentication for other applications. In our temporary email server, we only want the ◊acronym{IMAP} service:

◊code/block{
protocols = imap
}

Usually email servers support multiple users and have to manage a database with their credentials. This database might need to collaborate with other applications, for example, a web-client which allows users to change their passwords. We do not want this complexity, so we hard-code in the configuration file the credentials for a single user, ourselves:
