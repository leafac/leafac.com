...
◊new-thought{Modern operating system’s} ◊acronym{GUI}s make an analogy to an office environment. There is a desktop with some files organized in folders; and tools to work on them: text editors are analogous to pens and pencils, for example. Let us then consider this analogy when there is a requirement to track a project’s history over time. One could use, for example, a copying machine to record different versions as files change. Soon this results in the situation mentioned ◊reference['what-does-git-do]{when introducing version control}: there are many different versions of any given file and it is difficult to inspect the differences, attribute the changes to different people in the project, collaborate and experiment. Unsurprisingly, the exact same issues arise when using the computer.

To address this problem in an office, one could try a new system. After changing a file, one would take note of the change in a slip of paper, mentioning what was the previous content and what is the new content. One could use a paper tray to organize these slips of paper. After significant progress—which might mean anything from changing a single line to changing dozens of files—one could move these slips of paper into a box, label the box, and put the box in a cabinet for storage. The label in the box would contain the identification of the author of the changes, the current date and time, a high-level description of the box contents, an unique identifier for the current box and a reference to the identifier of the previous box.

With this system, the result would be a chain of boxes that record the history of the project in an organized and predictable manner. There is still only one copy of the files on which to work, but it is still possible to recreate the history and recollect who changed what, when and why, by looking in the boxes.

◊margin-note{◊svg{commit.svg}}

Maintaining this system is a lot of work, though. To recover the state of the files at a particular moment, it is necessary to look in the cabinet for the relevant series of boxes, open each of them in order and apply the recorded changes to the files. To find who is responsible for a particular change, one has to open each of the boxes to find the relevant slip of paper. It would be desirable to hire a person whose job is to manage these processes.
