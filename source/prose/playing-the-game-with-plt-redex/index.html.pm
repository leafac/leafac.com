...
◊margin-note{Note that we are using Racket’s support for Unicode identifiers.}
...
◊margin-note{Note that, in Racket, different kinds of brackets—◊code/inline{()}, ◊code/inline{[]} and ◊code/inline{{}}—mean the same. Which one to use is a matter of readability.}
...
◊margin-note{Note how closely the code resembles the ◊link["https://en.wikipedia.org/wiki/Peg_solitaire#Play"]{game specification}.}
